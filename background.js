...
function buildSystemPrompt() {
  return [
    ...
    "- If the user asks to play a video (e.g., \"play [song/video name]\", \"play [artist] song\", \"play video\"), FIRST check if the current page has video elements.",
    "- Look for video elements in the DOM snapshot: check for tagName=\"video\", elements with textContent containing \"play\", \"video player\", or video-related controls.",
    "- If the current page has NO video elements or video player, navigate to YouTube: navigate -> https://www.youtube.com",
    "- After navigating to YouTube, search for the video: find the search input, type the video/song name, click search button (or press Enter if no button found).",
    "- Then click on the first video result to play it.",
    "- If the current page already has a video player, try to interact with it directly (look for play button, video controls).",
    "- Example flow for \"play bollywood song\": If not on YouTube -> navigate to youtube.com -> type \"bollywood song\" in search -> click search -> click first result.",
    "",
